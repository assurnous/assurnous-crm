...
const Leads = () => {
  ...
  useEffect(() => {
    const getUserData = async () => {
      try {
        ...
        if (activeFilter !== "tous") {
          setFilteredData(
            sortedData.filter((item) => item.statut === activeFilter)
          );
        }
      } catch (err) {
        setError("Failed to fetch data");
  ...
  useEffect(() => {
    const dataToUse = filteredData.length > 0 ? filteredData : chatData;
  ...
  const handleAssign = async (values) => {
    try {
      ...
      const endpoint =
        assignType === "commercial" ? "/assign-leads" : "/assign-manager";
      const field = assignType === "commercial" ? "commercial" : "manager";
      const idField =
        assignType === "commercial" ? "commercialId" : "managerId";
      ...
      const updatedLeads = chatData.map((lead) => {
        if (selectedLeads.includes(lead._id)) {
          const sourceList =
            assignType === "commercial" ? commercials : managers;
          const assignedPerson = sourceList.find(
            (person) => person._id === values[field]
          );
          return {
            ...lead,
      ...
      message.success(
        `Clients affectés au ${
          assignType === "commercial" ? "commercial" : "manager"
        } avec succès`
      );
      ...
      message.error(
        `Échec de l'affectation des clients au ${
          assignType === "commercial" ? "commercial" : "manager"
        }`
      );
    }
  ...
  const handleUnassign = async () => {
    try {
      ...
      const endpoint =
        assignType === "commercial" ? "/unassign-leads" : "/unassign-manager";
      const field = assignType === "commercial" ? "commercial" : "manager";
      ...
      message.error(
        `Échec de la désaffectation des clients du ${
          assignType === "commercial" ? "commercial" : "manager"
        }`
      );
    }
  ...
  const columns = [
    ...
    // },
    {
      ...
      key: "comment",
      render: (text, record) => {
        const comment = record.comment || record.commentaire || "";
        ...
        if (words.length === 0 || words[0] === "") {
          return (
            ...
            >
              <div className="font-medium truncate max-w-xs text-gray-400 italic">
                Aucun commentaire
              </div>
            </div>
          );
        ...
        return (
          <div
            ...
            onClick={() => handleLeadClick(record)}
          >
            <div className="font-medium truncate max-w-xs" title={comment}>
              {displayText}
              {hasMoreWords && <span className="text-gray-500">...</span>}
            </div>
          </div>
        );
      },
    ...
    <div className="md:p-4 p-1 w-full">
      ...
      <Card className="mb-4">
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Tooltip
              title={
                canAssignToManager
                  ? ""
                  : "Seuls les clients non affectés peuvent être assignés à un manager"
              }
            >
              <Button
                type="primary"
          ...
          <Col xs={24} sm={12} md={6}>
            <Tooltip
              title={
                canUnassignFromManager
                  ? ""
                  : "Seuls les clients affectés à un manager peuvent être désaffectés"
              }
            >
              <Button
                icon={<UserDeleteOutlined />}
      ...
      <div className="mb-4 p-4 flex items-center rounded-md gap-4 bg-white shadow-sm">
        ...
        <div className="ml-auto flex items-center gap-2">
          <span className="font-thin text-gray-600">Page</span>
          <Select
            value={currentPage}
            onChange={handlePageChange}
            className="w-20 border-gray-300"
          ...
          <span className="font-thin text-gray-600">sur {totalPages}</span>
        </div>
      </div>
      ...
      >
        <div className="mb-4 p-3 bg-yellow-50 rounded border border-yellow-200">
          <div className="text-sm text-yellow-700">
            ...
            affectés au {assignType === "commercial" ? "commercial" : "manager"}{" "}
            sélectionné.
          </div>
        </div>
        <Form form={assignForm} onFinish={handleAssign}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: `Veuillez sélectionner un ${
                  assignType === "commercial" ? "commercial" : "manager"
                }`,
              },
            ]}
          >
            ...
            >
              {(assignType === "commercial" ? commercials : managers).map(
                (person) => (
      ...
      {/* Unassign Modal */}
      <Modal
        title={`Désaffecter les clients du ${
          assignType === "commercial" ? "Commercial" : "Manager"
        }`}
          ...
          <Form.Item>
            <div className="mb-4 p-3 bg-orange-50 rounded border border-orange-200">
              <div className="text-sm text-orange-700">
                ...
                {selectedLeads.length} client(s) du{" "}
                {assignType === "commercial" ? "commercial" : "manager"} ?
              </div>
            </div>
            <Space>
