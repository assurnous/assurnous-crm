...
const userVilles = (digitalResponse?.userVilles || []).map((v) =>
  String(v).toUpperCase()
        ...
        if (roleLower === "admin") {
          ...
          // 1) Get the user's allowed agences from /datas (same source as the digital page)
          const allowedAgences = (digitalResponse?.userVilles || []).map((v) =>
            String(v).toUpperCase()
          );
          console.log("Allowed agences for this user:", allowedAgences);
        
          // 2) Helper: does this record belong to one of the allowed agences?
          const matchesAgence = (record) => {
            if (!record) return false;
        
            // Explicit agence on the record
            const explicit = (record.agence || "").trim().toUpperCase();
            if (["LENS", "VALENCIENNES", "LILLE"].includes(explicit)) {
              return allowedAgences.includes(explicit);
            }
        
            // Fallback: infer from postal code
            const cp = String(record.code_postal || record.codepostal || "")
              .trim()
              .replace(/\s/g, "");
            if (cp.startsWith("59")) return allowedAgences.includes("LILLE");
            if (cp.startsWith("62")) return allowedAgences.includes("LENS");
        
            return false;
          };
          console.log("=== HAMZA DEBUG ===", {
            roleLower,
            allowedAgences,
            rawUserVilles: digitalResponse?.userVilles,
            digitalResponseKeys: Object.keys(digitalResponse || {}),
            totalClientsBeforeFilter: clients.length,
            totalDigitalBeforeFilter: digitalClients.length,
            sampleFirst3Clients: (clients || []).slice(0, 3).map((c) => ({
              nom: c.nom,
              prenom: c.prenom,
              agence: c.agence,
              code_postal: c.code_postal,
              matchesLENS: allowedAgences.includes("LENS"),
              matchesAgenceFn: matchesAgence(c),
            })),
          });
          // If the user has access to all three, don't filter — show everything
          ...
          if (hasAccessToAll) {
          ...
          } else if (allowedAgences.length === 0) {
            // No agences determined → safest is empty
            console.warn(`${roleLower} has no agences — returning empty sets`);
            clients = [];
            digitalClients = [];
            contrats = [];
            reclamations = [];
            ...
            // Filter clients and digital clients by agence
            clients = clients.filter(matchesAgence);
            digitalClients = digitalClients.filter(matchesAgence);
        
            // For contracts / reclamations / sinistres — filter by the agence
            // of the linked client (via gestionnaire or lead ref)
            // Simple approach: if the record has its own agence, use it.
            // Otherwise, keep it if its linked lead/client matches.
            contrats = contrats.filter((c) => {
              // Try direct agence
              if (c.agence) return allowedAgences.includes(String(c.agence).toUpperCase());
              // Try the populated lead (if present)
              if (c.lead && typeof c.lead === "object") return matchesAgence(c.lead);
              // No info → exclude for safety
              return false;
            });
        
            reclamations = reclamations.filter((r) => {
              if (r.agence) return allowedAgences.includes(String(r.agence).toUpperCase());
              if (r.lead && typeof r.lead === "object") return matchesAgence(r.lead);
              if (r.client && typeof r.client === "object") return matchesAgence(r.client);
              return false;
            });
        
            sinistres = sinistres.filter((s) => {
              if (s.agence) return allowedAgences.includes(String(s.agence).toUpperCase());
              if (s.lead && typeof s.lead === "object") return matchesAgence(s.lead);
              if (s.client && typeof s.client === "object") return matchesAgence(s.client);
              return false;
            });
        
            console.log(`${roleLower} filtered counts (by agence):`, {
              agences: allowedAgences,
              clients: clients.length,
              digitalClients: digitalClients.length,
              contrats: contrats.length,
              reclamations: reclamations.length,
