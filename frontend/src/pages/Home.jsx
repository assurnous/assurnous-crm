      ...
      try {
        ...
        if (roleLower === "admin") {
          ...
          contrats = contrats.filter((contract) => {
            ...
            return (
              matchIds.includes(gestionnaireId) || matchIds.includes(createdById)
            );
          });
  ...
  const processStats = (reclamations, sinistres, clients, contrats, digitalClients = []) => {
    ...
    contrats.forEach((contract) => {
      ...
      if (contract.insurer) {
        ...
        assureurCommissions[contract.insurer].totalCommission += commission;
        assureurCommissions[contract.insurer].totalPrevisionnel += previsionnel;
        assureurCommissions[contract.insurer].totalBrokerageFees += brokerageFees;
        assureurCommissions[contract.insurer].count++;
      }
      ...
      if (contract.riskType) {
        ...
        risqueCommissions[contract.riskType].totalCommission += commission;
        risqueCommissions[contract.riskType].totalPrevisionnel += previsionnel;
        risqueCommissions[contract.riskType].totalBrokerageFees += brokerageFees;
        risqueCommissions[contract.riskType].count++;
      }
    ...
    const sumCommissions = (obj) =>
      Object.values(obj).reduce(
        ...
        { totalCommission: 0, totalPrevisionnel: 0, totalBrokerageFees: 0, count: 0 }
      );
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="bg-white p-4 rounded-lg shadow-sm border">
        ...
        {userRole === "Commercial" && (
          <p className="text-sm text-gray-600">Affichage de vos statistiques personnelles</p>
        )}
        {userRole === "Manager" && <p className="text-sm text-gray-600">Vue d'ensemble</p>}
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="px-4 py-3 ml-6">
              <div className="flex items-center">
                <div>
                  <div className="text-xs text-gray-600">Prime TTC total</div>
                  <div className="text-sm font-bold">{stats.primeTotals.primeTTC}</div>
                </div>
              </div>
            </div>
          </div>
