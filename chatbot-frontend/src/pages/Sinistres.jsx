  ...
  const applyFilters = (filterValues) => {
    ...
    if (filterValues.periode && filterValues.periode[0] && filterValues.periode[1]) {
      result = result.filter(item => {
        ...
        return sinistreDate >= filterValues.periode[0] && 
    ...
    if (filterValues.risque && filterValues.risque !== "tous") {
      result = result.filter(
        item => item.risque === filterValues.risque
      );
    }
    ...
    //   result = result.filter(item => {
    //     const clientName = `${item.sinistreDetails?.nom || ''} ${item.sinistreDetails?.prenom || ''}`.toLowerCase();
    //     const contractNumber = item.contratDetails?.contractNumber?.toLowerCase() || '';
    ...
    //       (item.risque?.toLowerCase().includes(searchTerm)) ||
    //       (item.typeSinistre?.toLowerCase().includes(searchTerm))
    //     );
    //   });
    // }
...
if (filterValues.search) {
  ...
  result = result.filter(item => {
    ...
    // Address information
    const address = `${item.sinistreDetails?.numero_voie || ''} ${item.sinistreDetails?.code_postal || ''} ${item.sinistreDetails?.ville || ''}`.toLowerCase();
  ...
  const handleDelete = async (record) => {
    try {
      ...
      setAllSinistres(allSinistres.filter((sinistre) => sinistre._id !== record._id));  
      setFilteredSinistres(filteredSinistres.filter((sinistre) => sinistre._id !== record._id));
    } catch (error) {
  ...
  const handleEdit = async (record) => {
    ...
    // Reset form first
    form.resetFields();
    const clientData = clients.find(c => c._id === record.sinistreId) || 
    ...
    const formValues = {
      ...
      dateDeclaration: record.dateDeclaration ? dayjs(record.dateDeclaration) : null,
    ...
    if (record.contratExist === "oui") {
      ...
      const matchingContrat = contrats.find(contrat => 
    ...
    // },
    {
      ...
      render: (_, record) => {
        ...
        if (isClickable) {
          ...
          clientName = `${record.sinistreNom || ''} ${record.sinistrePrenom || ''}`.trim() || 
                       record.sinistreInput || "N/A";
        }
  
        return (
          <span 
            onClick={() => isClickable && handleLeadClick(
              record.sinistreDetails?._id || 
              (typeof record.leadId === 'object' ? record.leadId._id : record.leadId)
            )}
            style={{ 
  ...
  {sinistreExist === "non" && (
    <>
      <Form.Item
        ...
        name="sinistreNom"
        rules={[{ required: true, message: "Le champ nom du sinistré est obligatoire" }]}
      >
      ...
      <Form.Item
        ...
        name="sinistrePrenom"
        rules={[{ required: true, message: "Le champ prénom du sinistré est obligatoire" }]}
      >
      ...
      <Form.Item
       ...
       name="sinistreInput"
       rules={[{ required: true, message: "Le champ numéro de sinistre est obligatoire" }]}
      >
        <Input placeholder="Entrez le numéro de sinistre" />
      </Form.Item>
    </>
    ...
    <>
      ...
      {contratExist === "oui" && (
        <Form.Item
          label="Contrat"
          name="contratId"
          rules={[{ required: true, message: "Le champ contrat est obligatoire" }]}
        >
          <Select
            placeholder={loadingContrats ? "Chargement..." : "-- Choisissez un contrat --"}
      ...
      {contratExist === "non" && (
        <Form.Item
          label="Numéro de contrat"
          name="contratNumber"
          rules={[{ required: true, message: "Le champ numéro de contrat est obligatoire" }]}
        >
      ...
      return (
        <Option
          ...
          value={JSON.stringify({
            id: user._id,
            model: user.userType === 'admin' ? 'Admin' : 'Commercial'
          })}
        >
          {displayName} ({user.userType === "admin" ? "Admin" : "Commercial"})
        </Option>
